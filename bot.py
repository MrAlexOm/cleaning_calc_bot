...
# ---------- CONFIG ----------
TOKEN = "8162969073:AAFH5BPDIWNHqVuzfzbHrqFZsBTxIsmYpK4"  # Вставь свой токен
ADMIN_ID = 6181649972 
...
HOURLY_RATE = 450
DISTANCE_FEE = {"Кемер": 500, "Белек": 500, "Анталья": 0}
...
# Из списка удалены санузлы и балконы, так как о них бот спрашивает отдельно
EXTRAS = {
...
def calculate_total(chat_id):
    ...
    # Если почасовая - расчет не делаем, возвращаем метку
...
def layout_set(m):
    ...
    if m.text in ["6+1", "7+1"]:
        bot.send_message(m.chat.id, "🏢 Большая площадь! Для точного расчета напишите менеджеру: " + WHATSAPP_LINK)
        return
    if m.text == "2+1":
...
def finalize(cid):
    ...
    # Формируем сообщение в зависимости от типа уборки
    if res.get("is_hourly"):
        ...
        price_text = f"💰 *ИТОГО: ~{res['total']} TL*\n👥 Рекомендуем: {res['c']} чел. на {res['h']} ч."
...
def send_adm(m):
    ...
    try:
        ...
        bot.send_message(cid, "✅ **Заявка принята!**\nМенеджер свяжется с вами.\n\n"
                             "📸 [Instagram](https://www.instagram.com/cleanteam.antalya)\n"
                             "⚡️ [WhatsApp](" + WHATSAPP_LINK + ")", 
...
def def_m(m): bot.send_message(m.chat.id, "Нажмите /start")

if __name__ == "__main__":
    print("CleanTeam Bot is running...")
